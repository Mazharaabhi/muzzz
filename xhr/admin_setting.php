...
if ($f == 'admin_setting' AND (Wo_IsAdmin() || Wo_IsModerator())) {
    ...
    if ($s == 'delete_message') {
        if (!empty($_GET['msg_id'])) {
            $message_id    = Wo_Secure($_GET['msg_id']);
    ...
    if ($s == 'edit_message') {
        if (!empty($_GET['msg_id']) && !empty($_GET['text'])) {
            $message_id    = Wo_Secure($_GET['msg_id']);
            $text    = Wo_Secure($_GET['text']);
            $query_one     = "SELECT * FROM " . T_MESSAGES . " WHERE `id` = {$message_id}";
            $sql_query_one = mysqli_query($sqlConnect, $query_one);
            if ($sql_query_one) {
                if (mysqli_num_rows($sql_query_one) == 1) {
                    $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
                    $query = mysqli_query($sqlConnect, "UPDATE " . T_MESSAGES . " SET `text` = '" . $text . "' WHERE `id` = {$message_id}");
                }
            }
            $data = array(
                'status' => 200
    ...
    if ($s == 'delete_comment') {
        if (!empty($_GET['msg_id'])) {
            $message_id    = Wo_Secure($_GET['msg_id']);
            $query_one     = "SELECT * FROM " . T_COMMENTS . " WHERE `id` = {$message_id}";
            $sql_query_one = mysqli_query($sqlConnect, $query_one);
            if ($sql_query_one) {
                if (mysqli_num_rows($sql_query_one) == 1) {
                    $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
                    $query = mysqli_query($sqlConnect, "DELETE FROM " . T_COMMENTS . " WHERE `id` = {$message_id}");
                    if ($query) {
                        if (isset($sql_fetch_one['media']) and !empty($sql_fetch_one['media'])) {
                            @unlink($sql_fetch_one['media']);
                            $delete_from_s3 = Wo_DeleteFromToS3($sql_fetch_one['media']);
    ...
    if ($s == 'edit_comment') {
        if (!empty($_GET['msg_id']) && !empty($_GET['text'])) {
            $comment_id    = Wo_Secure($_GET['msg_id']);
            $text    = Wo_Secure($_GET['text']);
            $query_one     = "SELECT * FROM " . T_COMMENTS . " WHERE `id` = {$comment_id}";
            $sql_query_one = mysqli_query($sqlConnect, $query_one);
            if ($sql_query_one) {
                if (mysqli_num_rows($sql_query_one) == 1) {
                    $sql_fetch_one = mysqli_fetch_assoc($sql_query_one);
                    $query = mysqli_query($sqlConnect, "UPDATE " . T_COMMENTS . " SET `text` = '" . $text . "' WHERE `id` = {$comment_id}");
                }
            }
            $data = array(
                'status' => 200
        ...
        if (!empty($_GET['msg_id']) && !empty($_GET['text'])) {
            ...
            if ($sql_query_one) {
                if (mysqli_num_rows($sql_query_one) == 1) {
                    ...
                    $ms_data['from_id'] = $sql_fetch_one['to_id'];
                    $ms_data['to_id'] = $sql_fetch_one['from_id'];
                    //$ms_data['reply_id'] = $message_id;
                    $ms_data['text'] = $text;
                    $ms_data['media'] = "";
                    $ms_data['stickers'] = "";
                    $ms_data['time'] = time();
